...
RSpec.describe Device, type: :model do
  ...
  describe '#available?' do
    before do
      ...
      @issue_date =  Time.current + 3.hours
      @due_date = Time.current + 5.hours
    end

    context 'when no any previous lease are made' do
      it 'should return true' do
        expect(@device.available?(@issue_date, @due_date)).to be_truthy
      end
    end

    context 'when previous lease are made but not for the requested time' do
      it 'should return true' do
        FactoryGirl.create_list(:lease, @device_quantity, issue_date: @issue_date - 1.hours, due_date: @issue_date, item_id: @device.id)
        expect(@device.available?(@issue_date, @due_date)).to be_truthy
      end
    end
    ...
    context 'when their are previous leases is the requested time but less that quantity' do
      it 'should return true' do
        FactoryGirl.create_list(:lease, @device_quantity -1, issue_date: @issue_date, due_date: @due_date, item_id: @device.id)
        expect(@device.available?(@issue_date, @due_date)).to be_truthy
      end
    end
    ...
    context 'when their are previous lease is the requested time upto device quantity' do
      it 'should return false' do
        FactoryGirl.create_list(:lease, @device_quantity, issue_date: @issue_date, due_date: @due_date, item_id: @device.id)
        expect(@device.available?(@issue_date, @due_date)).to be_falsey
      end
    end
    ...
    context 'when their are previous lease upto device quantity but not complete time frame' do
      it 'should return true' do
        FactoryGirl.create_list(:lease, @device_quantity - 1, issue_date: @issue_date, due_date: @due_date, item_id: @device.id)
        FactoryGirl.create(:lease, issue_date: @issue_date, due_date: @due_date - 1.hours, item_id: @device.id)
